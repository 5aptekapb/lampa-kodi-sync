(function () {
    ...
    function showAboutPlugin() {
        // можно оставить пустой, либо вставить минимальное окно без проверки
        Lampa.Modal.open({
            title: 'Интерфейс MOD',
            html: '<div>Плагин работает</div>',
            size: 'large',
    ...
    function colorizeSeriesStatus() {
        function applyStatusColor(statusElement) {
            ...
            else if (statusText.includes('Відмін') || statusText.includes('Canceled')) { bgColor = statusColors.canceled.bg; textColor = statusColors.canceled.text; }
            ...
            else if (statusText.includes('чутках') || statusText.includes('Rumored')) { bgColor = statusColors.rumored.bg; textColor = statusColors.rumored.text; }
            else if (statusText.includes('Незабаром') || statusText.includes('Post')) { bgColor = statusColors.post.bg; textColor = statusColors.post.text; }
            if (bgColor) {
                $(statusElement).css({
            ...
            if (data.type === 'complite' && settings.enhance_detailed_info) {
                setTimeout(function() {
                    ...
                    details.find('span').each(function() {
                        ...
                        if (text.match(/Сезон(?:и)?:?\s*(\d+)/i) || text.match(/(\d+)\s+Сезон(?:а|ов)?/i)) {
                            seasonText = text;
